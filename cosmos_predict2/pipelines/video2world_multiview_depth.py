...
class Video2WorldMultiviewDepthPipeline(Video2WorldActionConditionedPipeline):
    ...
    def from_config(
        ...
        # Create a pipe
        pipe = Video2WorldMultiviewDepthPipeline(device=device, torch_dtype=torch_dtype)
        pipe.config = config
        pipe.precision = {
        ...
        assert (
            pipe.tokenizer.latent_ch + 4 == pipe.config.state_ch
    ...
    def get_data_and_condition(
        ...
        if not torch.is_floating_point(data_batch["depth"]):
            data_batch[IS_PREPROCESSED_KEY] = False
            self._normalize_video_databatch_inplace(data_batch, input_key="depth")
        if not torch.is_floating_point(data_batch["pred_depth"]):
            data_batch[IS_PREPROCESSED_KEY] = False
            self._normalize_video_databatch_inplace(data_batch, input_key="pred_depth")
    ...
    def decode(self, latent: torch.Tensor) -> torch.Tensor:
        latent_video = latent[:, :, :(16 + 1), :, :]
        return self.tokenizer.decode(latent_video / self.sigma_data)
    ...
    def _get_data_batch_input(
        ...
        Args:
            ...
            depth:the input depth tensor(B, 1, T, H, W)
        Returns:
    ...
    def __call__(
        ...
        vid_input = video.unsqueeze(0)
        dpt_input = depth.unsqueeze(0)
        pred_input = pred_video.unsqueeze(0)
        pred_dpt_input = pred_depth.unsqueeze(0)
