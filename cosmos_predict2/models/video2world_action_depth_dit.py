    ...
    def forward(
        ...
        **kwargs,
        ...
        if not hasattr(self, 'dim_reduction'):
            # 在第一次调用时创建降维和升维层
            self.dim_reduction = nn.Linear(512, 32).to(device=x_B_T_H_W_D.device, dtype=x_B_T_H_W_D.dtype)
            self.dim_restoration = nn.Linear(32, 512).to(device=x_B_T_H_W_D.device, dtype=x_B_T_H_W_D.dtype)
        
        # 对输入数据进行降维
        original_shape = x_B_T_H_W_D.shape
        x_B_T_H_W_D_flat = x_B_T_H_W_D.view(-1, 512)  # 展平最后维度
        x_B_T_H_W_D_reduced = self.dim_reduction(x_B_T_H_W_D_flat)
        x_B_T_H_W_D_reduced = x_B_T_H_W_D_reduced.view(original_shape[0], original_shape[1], original_shape[2], original_shape[3], 32)
        ...
        # 对输出数据进行升维：从32维还原到512维
        restored_shape = x_B_T_H_W_D_reduced.shape
        x_B_T_H_W_D_reduced_flat = x_B_T_H_W_D_reduced.view(-1, 32)  # 展平最后维度
        x_B_T_H_W_D_restored = self.dim_restoration(x_B_T_H_W_D_reduced_flat)
        x_B_T_H_W_D_restored = x_B_T_H_W_D_restored.view(restored_shape[0], restored_shape[1], restored_shape[2], restored_shape[3], 512)
