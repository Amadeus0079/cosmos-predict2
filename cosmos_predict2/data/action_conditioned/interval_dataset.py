...
class IntervalDataset(Dataset):
    def __init__(
        ...
        """Dataset class with "cheat" first frame for novel view synthesis.

        Similar to NovelViewDataset, but the first frame of pred_video is the actual
        RGB image from the pred_cam at the global first frame (frame 0) of the trajectory,
        while remaining frames are rendered from the merged GT pointcloud.
        ...
        Returns dict with:
            ...
            - pred_video: First frame is actual RGB from global frame 0,
                         remaining frames are rendered from GT pointcloud [C,T,H,W]
    ...
    def _get_ref_frame(self, label, frame_id, cam_id):
        video_path = label["videos"][cam_id]["video_path"]
        video_path = os.path.join(self.video_path, video_path)
        episode_dir = os.path.dirname(video_path)
        
        ref_image_dir = os.path.join(episode_dir, f"{cam_id}_ref")
        img_path = os.path.join(ref_image_dir, f"{(frame_id+1):06d}.png")
        
        bgr_img = cv2.imread(img_path)
        rgb = cv2.cvtColor(bgr_img, cv2.COLOR_BGR2RGB)  # (h, w, c)
        rgb = np.transpose(rgb, (2, 0, 1))
        frame = torch.from_numpy(rgb)  # (c, h, w)
