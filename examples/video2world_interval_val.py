...
def setup_pipeline(args: argparse.Namespace):
    ...
    # Load models
    pipe = Video2WorldCheatviewPipeline.from_config(
        config=config,
        dit_path=dit_path,
...
def process_single_generation(
    ...
    print(f"input_video: {input_video.shape}")  # [C, T, H, W]
    print(f"ori actions: {actions.shape}")
    print(f"actions: {actions[:chunk_size].shape}")
    print(f"input_condition: {input_condition.shape}")  # [C, T, H, W]
    
    # input_condition[:, 0] = input_condition[:, 1]
...
def generate_video(args: argparse.Namespace, pipe: Video2WorldCheatviewPipeline, val_dataset: NovelViewDataset) -> None:
    ...
    for j in range(8600, min(args.max_samples, total_samples), args.sample_interval):
        for i in range(j, j + 8):
            ...
            output_path = f"output/interval/interval_{args.model_type}_{dit_name}_sample{args.num_sampling_step}_{randomview_name}_idx{i}_rv{randomview_id}.mp4"
...
if __name__ == "__main__":
    ...
    # val_dataset = instantiate(flexiv_val_dataset)
    val_dataset = instantiate(robocasa_interval_val_dataset)

    log.info(f"Loaded FlexivDataset with {len(val_dataset)} samples")
