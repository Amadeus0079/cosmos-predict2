...
def process_episode(ep_name, data_group, episode_to_id, set_videos_dir, set_annotations_dir, split, cam_names):
    ...
    action_data = {
        ...
        "extrinsic_matrix": {cam_name: episode['cam_info'][cam_name + '_extrinsics'][:].tolist() for cam_name in cam_names},
        "intrinsic_matrix": {cam_name: episode['cam_info'][cam_name + '_intrinsics'][:].tolist() for cam_name in cam_names},
...
if __name__ == '__main__':
    ...
    parser.add_argument("--hdf5-path", type=str, 
                  default="/inspire/hdd/project/robot-reasoning/xiangyushun-p-xiangyushun/zichen/robocasa/datasets/v0.1/single_stage/kitchen_pnp/PnPCounterToSink/mg/2024-05-04-22-14-06_and_2024-05-07-07-40-17/demo_im128_ep300_cam2+1.hdf5", 
                  help="输入的RoboCasa HDF5文件路径。")
    parser.add_argument("--output-dir", type=str, 
                  default="/inspire/hdd/project/robot-reasoning/xiangyushun-p-xiangyushun/zichen/cosmos-predict2/datasets/robocasa2+4", 
                  help="转换后数据集的输出根目录。")
    parser.add_argument("--domain-name", type=str, default="PnPCounterToSink",
                  help="为这个新数据集指定的领域名称。")
