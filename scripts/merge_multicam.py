...
def process_episode(ep_name, data_group, supple_group, episode_to_id, set_videos_dir, set_annotations_dir, split, cam_names):
    ...
    try:
        ...
        original_states = episode['actions_abs'][:][index]
        ...
        action_data = {
            ...
            "texts": [lang],
            "videos": [{"video_path": f"videos/{split}/{episode_id}/rgb.mp4"}],
        ...
        for cam_name in cam_names:
            # 获取该摄像头的所有帧并调整大小
            frames = sp_episode['obs'][cam_name][:]
            resized_frames = []
            for frame in frames:
                # 转换为BGR并调整为256x256
            ...
            cam_frames[cam_name] = resized_frames

        # 确保我们有四个摄像头
        if len(cam_frames) == 4:
            # 假设所有摄像头的帧数相同
            num_frames = len(cam_frames[cam_names[0]])
            
            # 创建输出目录和视频写入器
            episode_video_dir = os.path.join(set_videos_dir, f"{episode_id}")
            os.makedirs(episode_video_dir, exist_ok=True)
            video_path_abs = os.path.join(episode_video_dir, "rgb.mp4")
            
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            output_fps = 6.0
            # 输出尺寸为512x512
            out = cv2.VideoWriter(video_path_abs, fourcc, output_fps, (512, 512))
            
            # 逐帧处理并拼接
            for i in range(num_frames):
                # 获取四个摄像头的对应帧
                frame1 = cam_frames[cam_names[0]][i]
                frame2 = cam_frames[cam_names[1]][i]
                frame3 = cam_frames[cam_names[2]][i]
                frame4 = cam_frames[cam_names[3]][i]
                
                # 按2x2拼接：先横向拼接，再纵向拼接
                top_row = cv2.hconcat([frame1, frame2])  # 上排：256x512
                bottom_row = cv2.hconcat([frame3, frame4])  # 下排：256x512
                combined = cv2.vconcat([top_row, bottom_row])  # 整体：512x512
                
                out.write(combined)
            
            out.release()
            print(f"已生成拼接视频: {video_path_abs}")
        else:
            print(f"需要4个摄像头，实际找到{len(cam_frames)}个")
    
    except Exception as e:
        return f"处理轨迹 {ep_name} 时发生错误: {str(e)}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output-dir", type=str, 
                  ...
                  help="转换后数据集的输出根目录。")
    parser.add_argument("--domain-name", type=str, default="robocasa_jigsaw", 
                  help="为这个新数据集指定的领域名称。")
    ...
    parser.add_argument("--cam-names",
                        ...
                        default=[
                            ...
                            "robot0_handview_right_image",
                            "robot0_handview_front_image",
